...
def test_Group_subcommand_decorator(subcommand_cls, assign_to_section):
    ...
    method_name = "group" if issubclass(subcommand_cls, cloup.Group) else "command"
    ...
    subcommand = decorator(
        subcommand_name, section=section_arg, cls=subcommand_cls, help="Help"
    )(new_dummy_func())
...
def test_align_sections_context_setting(runner, ctx_value, cmd_value):
    ...
    cmd_kwargs = pick_non_missing(
        dict(align_sections=cmd_value, context_settings=cxt_settings)
    )
