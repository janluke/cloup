...
def test_constraints_are_checked_according_to_protocol(
    ...
    constraints = [Mock(spec_set=Constraint, wraps=FakeConstraint()) for _ in range(3)]
    ...
    @cloup.option_group(
        "first", cloup.option("--a"), cloup.option("--b"), constraint=constraints[0]
    )
    @cloup.option_group(
        "second", cloup.option("--c"), cloup.option("--d"), constraint=constraints[1]
    )
...
@pytest.mark.parametrize("command_type", ["command", "group"])
@pytest.mark.parametrize(
    "cmd_value", [MISSING, None, True, False], ids=lambda val: f"cmd_{val}"
)
@pytest.mark.parametrize(
    "ctx_value", [MISSING, None, True, False], ids=lambda val: f"ctx_{val}"
)
def test_constraints_are_shown_in_help_only_if_feature_is_enabled(
    ...
    cxt_settings = pick_non_missing(
        dict(
            show_constraints=ctx_value,
            terminal_width=80,
        )
    )
    cmd_kwargs = pick_non_missing(
        dict(show_constraints=cmd_value, context_settings=cxt_settings)
    )
    ...
    result = runner.invoke(cmd, args=["--help"], catch_exceptions=False, prog_name="test")
    out = result.output
...
def test_group_constraints_doesnt_prevent_displaying_help_in_subcommand(runner):
    @cloup.group()
    @cloup.option_group(
        "Credentials", require_all(cloup.option("--user"), cloup.option("--password"))
    )
    ...
    @cli.group()
    @cloup.option_group(
        "Required", require_one(cloup.option("--foo"), cloup.option("--bar"))
    )
