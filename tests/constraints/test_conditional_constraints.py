...
class TestAllSet:
    ...
    def test_descriptions(self, sample_cmd):
        ...
        assert AllSet("arg1", "flag").desc(ctx) == "ARG1 and --flag are both set"
        assert AllSet("arg1", "flag").neg_desc(ctx) == "ARG1 and --flag are not both set"

        assert (
            AllSet("arg1", "flag", "int_opt").desc(ctx)
            == "ARG1, --flag and --int-opt are all set"
        )
        assert (
            AllSet("arg1", "flag", "int_opt").neg_desc(ctx)
...
class TestAnySet:
    ...
    def test_descriptions(self, sample_cmd):
        ...
        assert AnySet("arg1", "flag").desc(ctx) == "either ARG1 or --flag is set"
        assert AnySet("arg1", "flag").neg_desc(ctx) == "neither ARG1 nor --flag is set"

        assert (
            AnySet("arg1", "flag", "int_opt").desc(ctx)
            == "any of ARG1, --flag and --int-opt is set"
        )
        assert (
            AnySet("arg1", "flag", "int_opt").neg_desc(ctx)
