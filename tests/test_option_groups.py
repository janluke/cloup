...
def test_option_groups_are_correctly_displayed_in_help(
    ...
    cmd = get_example_command(
        tabular_help=tabular_help, align_option_groups=align_option_groups
    )
...
def test_option_group_constraints_are_checked(runner, get_example_command):
    ...
    error_prefix = (
        "Error: when --three is set, at least 1 of the following parameters must be set"
    )
...
def test_option_group_decorator_raises_if_group_is_passed_to_contained_option():
    with pytest.raises(ValueError):
        ...
        @option_group(
            "a group", cloup.option("--opt", group=OptionGroup("another group"))
        )
        def f(opt):
            pass
...
def test_align_option_groups_context_setting(runner, ctx_value, cmd_value):
    ...
    cmd_kwargs = pick_non_missing(
        dict(align_option_groups=cmd_value, context_settings=cxt_settings)
    )
...
def test_that_neither_optgroup_nor_its_options_are_shown_if_optgroup_is_hidden(runner):
    @cloup.command("name")
    @cloup.option_group("Hidden group", cloup.option("--one"), hidden=True)
    def cmd():
        pass
...
def test_option_group_with_constrained_subgroups(runner):
    ...
    @option_group(
        "Some options",
        RequireAtLeast(1)(option("-a", is_flag=True), option("-b", is_flag=True)),
        mutually_exclusive(
...
def test_option_groups_raises_if_input_decorator_add_an_argument():
    with pytest.raises(
        ...
        option_group(
            "Title", mutually_exclusive(cloup.argument("arg"), cloup.option("--opt"))
        )(new_dummy_func())
