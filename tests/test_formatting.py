...
@parametrize(
    ...
    pytest.param(RowSepIf(multiline_rows_are_at_least(3)), "", id="empty_line"),
    pytest.param(
...
def test_formatter_settings_creation():
    ...
    assert HelpFormatter.settings(indent_increment=4, col_spacing=3) == dict(
...
def test_write_text_with_styles():
    ...
    EXPECTED = "\n".join(indentation + style(line) for line in WRAPPED.splitlines())
